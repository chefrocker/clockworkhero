            ...
            <div style={{border: '1px solid var(--border-color)', borderRadius: '12px', overflow: 'hidden'}}>
                {projects.map(p => (
                    <div key={p.id} style={{display: 'flex', alignItems: 'center', padding: '15px', borderBottom: '1px solid var(--border-color)', background: 'var(--panel-bg)', gap: '15px'}}>
                        ...
                        <input type="color" value={p.color || '#000000'} onChange={(e) => onUpdate(p.id, p.name, e.target.value, p.icon, p.iconType as any)} style={{width: '30px', height: '30px', border: 'none', cursor: 'pointer', borderRadius: '50%'}} />
                        <input className="input-text" value={p.name || ''} onChange={(e) => onUpdate(p.id, e.target.value, p.color, p.icon, p.iconType as any)} style={{flex: 1}} />
